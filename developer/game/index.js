function addImage () {
    ...
    const src = document.getElementById("imageInput");
    const images = document.getElementById("images");
    images.value += ` ${src.value}`;
    const html = `
        <div class="image" style="background-image: url('${src.value}'); background-size: cover" onclick=(removeImage(this))>
...
function removeImage (thisImage) {
    const images = document.getElementById("images");
    images.value = images.value.replace(` ${thisImage.style.backgroundImage.match(/url\("(.*?)"\)/)[1]}`, '');
    thisImage.remove();
}

function setCover () {
    const src = document.getElementById("cover");
